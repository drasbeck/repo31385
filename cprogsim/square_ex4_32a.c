...
typedef struct{ //input signals
		...
		// Odometry
		double th, th_old, x, x_old, y, y_old;
		} odotype;
...
int main()
{
  int running,n=0,arg,time=0,index=0,i,j;
...
*/
  ...
  //fprintf(data_file, "index,mission.time,mot.motorspeed_l,mot.motorspeed_r,odo.x,odo.y,odo.th");
  for (i = 0; i < DATA_LOG_ROW; i++) {
...
void update_odo(odotype *p)
{
  ...
  p->th_old = p->th;
  p->th = p->th_old + (delta_UR - delta_UL) / p->w;
  
  p->x_old = p->x;
  p->x = p->x_old + (delta_UR + delta_UL) * cos(p->th) / 2.0;
  
  p->y_old = p->y;
  p->y = p->y_old + (delta_UR + delta_UL) * sin(p->th) / 2.0;
...
if (p->cmd !=0){
   ...
   switch (p->curcmd){
	 ...
	 //printf("%f; %f\n", p->right_pos-p->startpos, p->angle*p->w/2);
	  if (p->right_pos-p->startpos < p->angle*p->w/2.0){
	      p->motorspeed_r=p->speedcmd;
	      p->motorspeed_l=-p->speedcmd;
	  }
	  else {	     
          ...
          //p->motorspeed_r=0;
	  if (p->left_pos-p->startpos < fabs(p->angle)*p->w/2.0){
	      p->motorspeed_l=p->speedcmd;
	      p->motorspeed_r=-p->speedcmd;
	  }
	  else {	     
