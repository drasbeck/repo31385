...
int main()
{
  ...
  while (running)
  {
	...
	if (fwd(0.50,0.3,mission.time)) {
      ...
      case ms_garTurn1:
	if (turn(-M_PI/2.0, 0.3, mission.time)) {
	  mission.state=ms_garFL1;
	}
      break;
      
      case ms_garFL1:
	if (followline("bm",0.60,0.3,mission.time)) {
	  mot.cmd = mot_stop;
	  mission.state=ms_garTurn2;
	}
      break;
